import { d as db, L as LoyaltyTier, j as Coupon, O as Order } from './_astro_db_Bcz5lWRF.mjs';
import { n as normalizeCouponCode } from './coupons_Cq1AMv1K.mjs';
...
function parseEuroToCents(value, optional = false) {
  const raw = String(value ?? "").trim().replace(",", ".");
...
function parseOptionalTierId(value) {
  const raw = String(value ?? "").trim();
  if (!raw) return null;
  const n = Number(raw);
...
function parseDateStart(value) {
  ...
  return /* @__PURE__ */ new Date(`${raw}T00:00:00`);
}
function parseDateEnd(value) {
  const raw = String(value ?? "").trim();
  if (!raw) return null;
  if (!/^\d{4}-\d{2}-\d{2}$/.test(raw)) return null;
  return /* @__PURE__ */ new Date(`${raw}T23:59:59.999`);
}
function isCouponKind(value) {
...
function resolveCouponValue(type, valueInput) {
  if (type === "PERCENT") {
...
const POST = async (context) => {
  ...
  if (intent === "create") {
    ...
    if (String(form.get("minSubtotalEur") ?? "").trim() && minSubtotalCents === null) {
      return context.redirect(withQuery(REDIRECT_PATH, {
        error: "invalid-min-subtotal"
      }));
    }
    if (String(form.get("maxUses") ?? "").trim() && maxUses === null) {
      return context.redirect(withQuery(REDIRECT_PATH, {
        error: "invalid-max-uses"
      }));
    }
    if (String(form.get("startsAt") ?? "").trim() && !startsAt || String(form.get("endsAt") ?? "").trim() && !endsAt) {
      return context.redirect(withQuery(REDIRECT_PATH, {
        error: "invalid-dates"
      }));
    }
  ...
  }).from(Coupon).where(eq(Coupon.id, couponId)).limit(1);
  if (!coupon) {
    return context.redirect(withQuery(REDIRECT_PATH, {
  ...
  if (intent === "delete") {
    const linkedOrders = await db.select({
      id: Order.id
    }).from(Order).where(eq(Order.couponId, couponId));
