...
function matchesArcadiaDeliveryArea(city, postalCode) {
  const normalizedCity = normalizeDeliveryCity(city);
  const normalizedPostalCode = String(postalCode ?? "").trim();
  return normalizedCity === "lloret de mar" && normalizedPostalCode === "17310";
}
function validateDeliveryAddressByArea(input, rule = DEFAULT_DELIVERY_AREA_RULE) {
  if (!rule.enabled) {
    return {
      enabled: false,
      allowed: true,
  ...
  return {
    ...
    message: allowed ? "Dirección dentro de la zona de reparto." : "Esta dirección está fuera de la zona de reparto. Solo repartimos en Lloret de Mar (17310)."
  };
}
...
export { getArcadiaAvailability as g, validateDeliveryAddressByArea as v };
