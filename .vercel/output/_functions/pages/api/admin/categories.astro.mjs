import { d as db, C as Category, c as Product, b as CategoryIngredient } from '../../../chunks/_astro_db_ChTDrd2j.mjs';
...
const POST = async (context) => {
  ...
  if (intent === "create") {
    ...
    const [slugMatch] = await db.select({
      id: Category.id
    }).from(Category).where(eq(Category.slug, slug)).limit(1);
    if (slugMatch) {
      return context.redirect(withQuery(REDIRECT_PATH, {
  ...
  if (intent === "delete") {
    const productLinks = await db.select({
      id: Product.id
    }).from(Product).where(eq(Product.categoryId, categoryId));
    if (productLinks.length > 0) {
      return context.redirect(withQuery(REDIRECT_PATH, {
        error: "in-use-products"
      }));
    }
    const ingredientLinks = await db.select({
      id: CategoryIngredient.id
    }).from(CategoryIngredient).where(eq(CategoryIngredient.categoryId, categoryId));
