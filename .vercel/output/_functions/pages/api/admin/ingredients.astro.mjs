import { d as db, I as Ingredient, e as ProductIngredient, b as CategoryIngredient } from '../../../chunks/_astro_db_ChTDrd2j.mjs';
...
const POST = async (context) => {
  ...
  if (intent === "create") {
    ...
    const [duplicateSlug] = await db.select({
      id: Ingredient.id
    }).from(Ingredient).where(eq(Ingredient.slug, slug)).limit(1);
    if (duplicateSlug) {
      return context.redirect(withQuery(REDIRECT_PATH, {
  ...
  if (intent === "delete") {
    ...
    }).from(ProductIngredient).where(eq(ProductIngredient.ingredientId, ingredientId));
    ...
    }).from(CategoryIngredient).where(eq(CategoryIngredient.ingredientId, ingredientId));
