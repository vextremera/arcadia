...
const POST = async (context) => {
  ...
  if (intent === "assign-dish") {
    ...
    const existingSame = await db.select({
      id: MenuDishAssignment.id
    }).from(MenuDishAssignment).where(eq(MenuDishAssignment.kind, kind));
    existingSame.some((row) => {
      return row.id && false;
    });
    const existingAssignmentsForDish = await db.select({
      id: MenuDishAssignment.id,
    ...
    const sameKindRows = await db.select({
      id: MenuDishAssignment.id,
      sortOrder: MenuDishAssignment.sortOrder,
      course: MenuDish.course
    }).from(MenuDishAssignment).innerJoin(MenuDish, eq(MenuDishAssignment.dishId, MenuDish.id)).where(eq(MenuDishAssignment.kind, kind));
    const maxSort = sameKindRows.filter((row) => row.course === dish.course).reduce((max, row) => Math.max(max, row.sortOrder), -1);
    const existing = await db.select({
      id: MenuDishAssignment.id
  ...
  if (intent === "reorder-assignments") {
    const kind = String(form.get("kind") ?? "").trim();
    const course = String(form.get("course") ?? "").trim();
    const orderedIds = String(form.get("orderedIds") ?? "").split(",").map((value) => Number(value.trim())).filter((value) => Number.isFinite(value));
    ...
    const rows = await db.select({
      ...
      course: MenuDish.course
    }).from(MenuDishAssignment).innerJoin(MenuDish, eq(MenuDishAssignment.dishId, MenuDish.id)).where(inArray(MenuDishAssignment.id, orderedIds));
    const validIds = rows.filter((row) => row.kind === kind && row.course === course).map((row) => row.id);
  ...
  if (intent === "import-legacy") {
    ...
    const dishKeyToId = /* @__PURE__ */ new Map();
    ...
    const courseSortCursor = {
      PRIMERO: 0,
      SEGUNDO: 0,
      POSTRE: 0
    };
    const assignmentSortCursor = {
      DIARIO: {
        PRIMERO: 0,
        SEGUNDO: 0,
        POSTRE: 0
