...
export default function CheckoutForm() {
  ...
  async function submit() {
    ...
    /**
     * Sin correo no hay dónde mandar la confirmación del pedido.
     *
     * Se comprueba a mano porque el botón no envía un formulario: es un
     * `type="button"` con onClick, así que el `required` del campo nunca
     * llegaría a dispararse por su cuenta.
     */
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(customerEmail.trim())) {
      alert(tt.requiredEmail);
      return;
    }
    ...
    <div class="mt-4 grid gap-4 sm:mt-6 sm:gap-6 xl:grid-cols-[minmax(0,1fr)_26rem] 2xl:grid-cols-[minmax(0,1fr)_30rem]">
        ...
        <section class="rounded-2xl border border-zinc-200 p-4 sm:p-5">
          ...
          <div class="mt-4 grid gap-3 lg:grid-cols-2">
            ...
            <div class="lg:col-span-2">
              {/*
                El correo dejó de ser opcional: es donde llega la confirmación
                del pedido, que la ley exige entregar en un soporte que el
                cliente pueda guardar y volver a leer.
              */}
              ...
              />
              <p class="mt-1.5 text-xs leading-5 text-zinc-500">{tt.emailNote}</p>
            </div>
          </div>
        </section>
      ...
      <aside class="xl:sticky xl:top-4 xl:h-fit">
        <div class="rounded-2xl border border-zinc-200 p-4 sm:p-5 2xl:p-6">
          ...
          {/*
            El precio que se muestra tiene que ser el final: con impuestos y con
            el reparto ya sumado. Decirlo evita la duda de si al final se añade
            algo más.
          */}
          <p class="mt-2 text-xs leading-5 text-zinc-500">{tt.taxNote}</p>
