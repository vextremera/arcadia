export type PublicAvailabilityLike = {
  pauseOrders?: boolean;
  forcePickup?: boolean;
  windows?: {
    delivery?: {
...
export function buildDeliveryMessage(availability: PublicAvailabilityLike) {
...
export function buildOperationalMessage(availability: PublicAvailabilityLike) {
  ...
  if (availability.forcePickup) {
    const note = safeNote(availability.sourceNotes?.delivery);
    return note
      ? `Ahora mismo trabajamos solo recogida. El reparto está temporalmente desactivado desde operativa. ${note}`
